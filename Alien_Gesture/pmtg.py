...
from raisim_gym.env.bin import pmtg
...
# print(cfg_abs_path)
cfg_abs_path = "/home/sonic/Project/Alien_Gesture/env/AlienGo_PMTG/pmtg/cfg.yaml"
...
env = VecEnv(pmtg.RaisimGymEnv(rsc_path, dump(cfg['environment'], Dumper=RoundTripDumper)))

# env = Environment(RaisimGymEnv(current_dir+"/env/urdf_aliengo", dump(cfg['environment'], Dumper=RoundTripDumper)))
# Get algorithm
if mode == 'train':
    saver = ConfigurationSaver(log_dir=log_dir+'/IN_PLACE',
                               save_items=[rsg_root+'/env/AlienGo_PMTG/pmtg/Environment.hpp', cfg_abs_path])
    model = PPO2(
    ...
    saver = ConfigurationSaver(log_dir=log_dir+'/IN_PLACE',save_items=[rsg_root+'/env/AlienGo_PMTG/pmtg/Environment.hpp', cfg_abs_path])
