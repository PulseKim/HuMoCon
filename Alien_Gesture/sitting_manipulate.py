...
from raisim_gym_s.env.bin import sitting
from raisim_gym_s.env.RaisimGymVecEnv import RaisimGymVecEnv as VecEnv
from raisim_gym_s.helper.raisim_gym_helper import ConfigurationSaver

from raisim_gym_s.algo.ppo2 import PPO2
...
# print(cfg_abs_path)
cfg_abs_path = "/home/sonic/Project/Alien_Gesture/env/AlienGo_Sitting/sitting/cfg.yaml"
...
if mode == "exp":
    cfg['environment']['num_envs'] = 256

task_path = os.path.dirname(os.path.realpath(__file__))
rsc_path = "/home/sonic/Project/Alien_Gesture/env/robots"
env = VecEnv(sitting.RaisimGymEnv(rsc_path, dump(cfg['environment'], Dumper=RoundTripDumper)))

# env = Environment(RaisimGymEnv(current_dir+"/env/urdf_aliengo", dump(cfg['environment'], Dumper=RoundTripDumper)))
# Get algorithm
if mode == 'train':
    saver = ConfigurationSaver(log_dir=log_dir+'/SittingManipulation',
                               save_items=[rsg_root+'/env/AlienGo_Sitting/sitting/Environment.hpp', cfg_abs_path])
    model = PPO2(
    ...
    # PPO run
    model.learn(total_timesteps=700000000, eval_every_n=cfg['environment']['eval_every_n'], log_dir=saver.data_dir, record_video=False)
    ...
    saver = ConfigurationSaver(log_dir=log_dir+'/SittingManipulation',save_items=[rsg_root+'/env/AlienGo_Sitting/sitting/Environment.hpp', cfg_abs_path])
    ...
    af = actionFilter(256)
    final_result = np.zeros(256)
    m_env = 20
    for epoch in range(m_env):
        went = 300
        af._ResetActionFilter()
        obs = env.reset()
        result = 300 * np.ones(256)
        flag = np.zeros(256)
        for i in range(300):
            # env.wrapper.showWindow()
            action, _ = model.predict(obs)
            action_t = af._FilterAction(action)
            obs, reward, done, infos = env.step(action_t)
            # obs, reward, done, infos = env.stepTest(action_t)
            # print(env.isTerminalState()[0][0])
            for j in range(done.shape[0]):
                if done[j] and flag[j] == 0:
                    result[j] = i
                    flag[j] = 1
        print(result)
        if epoch == 0:
    ...
    TC.print_time()
    print("mean: ", np.mean(final_result))
    print("se: ", np.std(final_result, ddof=1) / np.size(final_result))
