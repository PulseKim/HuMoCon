...
# Get algorithm
if mode == 'train':
    saver = ConfigurationSaver(log_dir=log_dir+'/SittingA1',
    ...
    model = PPO2(
        ...
        # n_steps=math.floor(cfg['environment']['max_time'] / 0.0166666666),
        ent_coef=0.0,
        learning_rate=5e-5,
        vf_coef=0.5,
        max_grad_norm=0.5,
    ...
    saver = ConfigurationSaver(log_dir=log_dir+'/SittingA1',save_items=[rsg_root+'/env/AlienGo_Sitting/sitting/Environment.hpp', cfg_abs_path])
    ...
    for i in range(100000):
        ...
        action_history = np.concatenate((action_history, obs[:, -12:]))
        ...
        if done[0]:
            ...
            print("Episode Reward: {:.2f}".format(running_reward))
            print("Episode Length", ep_len)
            np.savetxt( '/home/sonic/Project/Alien_Gesture/sitaction.txt', action_history ,fmt='%1.3f')
            running_reward = 0.0
            ep_len = 0
            break