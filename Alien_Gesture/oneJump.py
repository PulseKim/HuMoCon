...
from raisim_gym_onejump.env.bin import env
from raisim_gym_onejump.env.RaisimGymVecEnv import RaisimGymVecEnv as VecEnv
from raisim_gym_onejump.helper.raisim_gym_helper import ConfigurationSaver

from raisim_gym_onejump.algo.ppo2 import PPO2
from raisim_gym_s.archi.policies import MlpPolicy
...
# print(cfg_abs_path)
cfg_abs_path = "/home/sonic/Project/Alien_Gesture/env/AlienGo_OneJump/env/cfg.yaml"
...
# Get algorithm
if mode == 'train':
    saver = ConfigurationSaver(log_dir=log_dir+'/MappingOneJump',
                               save_items=[rsg_root+'/env/AlienGo_OneJump/env/Environment.hpp', cfg_abs_path])
    model = PPO2(
    ...
    for i in range(100000):
        ...
        running_reward += reward[0]
        # pd_gain = action[0,12: ]
        # total_pd = np.sum(pd_gain)
        total_torque = np.linalg.norm(torque)
        # plot.add(i,pd_gain, total_pd)
