...
from raisim_gym_lean.archi.policies import LstmPolicy, ActorCriticPolicy
...
class PPO2(ActorCriticRLModel):
    ...
    def learn(self, total_timesteps, callback=None, seed=None, log_interval=1, tb_log_name="PPO2",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            runner = Runner(env=self.env, model=self, n_steps=self.n_steps, gamma=self.gamma, lam=self.lam, n_envs = self.n_envs)
...
class Runner(AbstractEnvRunner):
    def __init__(self, *, env, model, n_steps, gamma, lam, n_envs):
        """
    ...
    def run(self, test_mode=False, record_video=False, video_name="", update = 0):
        ...
        for i in range(self.n_steps):
            ...
            action_t = self._FilterAction(clipped_actions)
            self.obs[:], rewards, self.dones, infos = self.env.step(action_t, visualize=test_mode)
            for info in infos:
            ...
            for i in range(len(self.dones)):
                if self.dones[i]:
                    self._ResetSingleActionFilter(i)
    ...
    def _BuildActionFilter(self):
        ...
        for _ in range(self.n_envs) : 
            ...
            env_filters.append(a_filter)
        return env_filters

    def _ResetActionFilter(self):
        for i in range(self.n_envs) : 
            self._action_filter[i].reset()
            self._action_filter[i].init_history(self.init_angle)

    def _ResetSingleActionFilter(self, i):
        self._action_filter[i].reset()
        self._action_filter[i].init_history(self.init_angle)

    def _FilterAction(self, action):
        filtered_action = np.zeros([self.n_envs, 12], dtype=np.float32)            
        for i in range(self.n_envs) : 
