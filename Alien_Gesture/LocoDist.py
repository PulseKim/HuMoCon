...
from raisim_gym_loco.env.bin import loco
from raisim_gym_loco.env.RaisimGymVecEnv import RaisimGymVecEnv as VecEnv
from raisim_gym_loco.helper.raisim_gym_helper import ConfigurationSaver

from new_alg.new_ppo2 import PPO2
from new_alg.new_policies import MlpPolicy
from new_alg.Model_Torch import *
import torch
...
def np_result_parser(result):
    scaler = np.array([0.803, 2.618, 0.8905, 0.803, 2.618, 0.8905, 0.803, 2.618, 0.8905, 0.803, 2.618, 0.8905], dtype = np.float32)
    shifter = np.array([0., 0.716, -0.0785, 0., 0.716,  -0.0785, 0., 0.716, -0.0785, 0., 0.716, -0.0785], dtype = np.float32)
    real = np.array([0., 0.855, -1.728, 0., 0.855, -1.728, 0., 0.855, -1.728, 0., 0.855, -1.728], dtype = np.float32)
    epsilon = 1e-6
    ...
    scaled_residual0 = np.add(np.multiply(scaler, joint_result0), shifter)
    scaled_angle0 = scaled_residual0 + real
...
# print(cfg_abs_path)
cfg_abs_path = "/home/sonic/Project/Alien_Gesture/env/A1_LocoDist/loco/cfg.yaml"
...
env = VecEnv(loco.RaisimGymEnv(rsc_path, dump(cfg['environment'], Dumper=RoundTripDumper)))

# env = Environment(RaisimGymEnv(current_dir+"/env/urdf_aliengo", dump(cfg['environment'], Dumper=RoundTripDumper)))
# Get algorithm
if mode == 'train':
    saver = ConfigurationSaver(log_dir=log_dir+'/DomainRand',
                               save_items=[rsg_root+'/env/A1_LocoDist/loco/Environment.hpp', cfg_abs_path])
    model = PPO2(
    ...
    model.set_env(env)
    model.set_update(36000)

    # tensorboard
    # Make sure that your chrome browser is already on.
    TensorboardLauncher_local(saver.data_dir + '/PPO2_1')
    # PPO run
    model.learn(total_timesteps=2000000000, eval_every_n=cfg['environment']['eval_every_n'], log_dir=saver.data_dir, record_video=False)
    ...
    dim_input = 96
    dim_output = 16
    rscdir2 = current_dir+ "/rsc"
    mapper_model = MLP_model_tanh(dim_input, dim_output)
    file_name = rscdir2 + '/map_reach/All.pt'
    file_name2 = rscdir2 + '/map_reach/Allcont.pt'
    device = torch.device('cuda')
    mapper_model.load_state_dict(torch.load(file_name, map_location=device))
    mapper_model.cuda()
    mapper_model.eval()
    contact_model = MLP_model_tanh(dim_input, 4)
    contact_model.load_state_dict(torch.load(file_name2, map_location=device))
    contact_model.cuda()
    contact_model.eval()
    env.kinect_init()
    obs = env.reset()
    ...
    for i in range(100000):
        ...
        y_result = mapper_model.forward(X).cpu().detach().numpy()
        contact = contact_model.forward(X).cpu().detach().numpy()
        kin_motion = np_result_parser(y_result).astype(np.float32)

        # Here, put scaled result ==> use perAgentTarget
        env.getMappedTarget(kin_motion, contact)
