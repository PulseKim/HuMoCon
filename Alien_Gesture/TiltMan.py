...
from raisim_gym_tiltman.env.bin import env
from raisim_gym_tiltman.env.RaisimGymVecEnv import RaisimGymVecEnv as VecEnv
from raisim_gym_tiltman.helper.raisim_gym_helper import ConfigurationSaver
...
parser.add_argument('--cfg', type=str, default=os.path.abspath(rscdir + "/cfg.yaml"),
...
# print(cfg_abs_path)
cfg_abs_path = "/home/sonic/Project/Alien_Gesture/env/A1_TiltMan/env/cfg.yaml"
...
# Get algorithm
if mode == 'train':
    saver = ConfigurationSaver(log_dir=log_dir+'/TiltMan',
                               save_items=[rsg_root+'/env/A1_TiltMan/env/Environment.hpp', cfg_abs_path])
    model = PPO2(
    ...
    for i in range(300):
        ...
        obs, reward, done, infos = env.stepTest(action_t)
        all_motion = np.concatenate((all_motion, obs[:,:]))
        ...
        if done[0]:
            ...
            running_reward = 0.0
            ep_len = 0
            np.savetxt( '/home/sonic/Project/Alien_Gesture/all_motion.txt', all_motion, fmt='%1.3f')
            break
