...
class ENVIRONMENT : public RaisimGymEnv {
 ...
 public:
      ...
      RaisimGymEnv(resourceDir, cfg), visualizable_(visualizable) 
  {
    ...
    x_distribution = std::uniform_real_distribution<double>(deg2rad(-5), deg2rad(5));
    y_distribution = std::uniform_real_distribution<double>(deg2rad(-5), deg2rad(5));
    z_distribution = std::uniform_real_distribution<double>(deg2rad(-5), deg2rad(5));
  ...
  float step(const Eigen::Ref<EigenVec>& action) final {
    /// action scaling
    gc_target.tail(12) = gc_init_.tail(12);
    ...
    if(current_count%throw_count == 0)
    {
      ballthrowing();
    }
    ...
    if(current_count%ori_count == 0)
    {
      update_body_angle();
    }
    interpolate_desired();
    interpolate_body_angle();
    current_count++;
  ...
  void curriculumUpdate(int update) final {
    ...
    else if(update >= curr9 && update <curr10) 
    {
      ...
      y_distribution = std::uniform_real_distribution<double>(deg2rad(-20), deg2rad(20));
  ...
  float endEffectorImitationErr()
  {
    ...
    gc_extended.segment(7,3) = gc_target.segment(4, 3);
  ...
  void updateObservation() {
    ...
    obScaled_[32] = desired_angleH;
    obScaled_[33] = desired_angleT;
    obScaled_[34] = desired_angleC;

    for(int i = 0 ; i < 4; ++ i){
      raisim::Vec<3> footHeight;
      alien_->getFramePosition(FR_FOOT + 3 * i, footHeight);
      obScaled_[35 + i] = footHeight[2] - 0.015;
    }
    gc_prev = gc_;
  }
