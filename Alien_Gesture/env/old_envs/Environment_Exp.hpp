...
std::uniform_real_distribution<double> thigh_distribution(deg2rad(-100), deg2rad(100));
// std::uniform_real_distribution<double> calf_distribution(-2.78, -0.65);
std::uniform_real_distribution<double> calf_distribution(-1.8, -1.0);
...
class ENVIRONMENT : public RaisimGymEnv {
 ...
 public:
  ...
  void put_boxes()
  {
    ...
    cubes.push_back(world_->addBox(0.1, 0.5, 0.3, 3 * mass));
  ...
  void updateObservation() {
    ...
    Eigen::Vector3d com = alien_->getCompositeCOM().e();
    obScaled_.segment(18, 2) = com.segment(0,2);
    for(int i = 0 ; i < 3; ++i)
    {
      obScaled_.segment(20 + 2 * i, 2) = feetPoses[i].segment(0,2);
    }
  }
