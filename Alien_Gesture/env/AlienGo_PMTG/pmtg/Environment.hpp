...
class ENVIRONMENT : public RaisimGymEnv {
 ...
 public:
      ...
      RaisimGymEnv(resourceDir, cfg), visualizable_(visualizable) 
  {
    ...
    actionStd_ << deg2rad(50), deg2rad(10), deg2rad(10), deg2rad(50), deg2rad(10), deg2rad(10), deg2rad(50), deg2rad(10), deg2rad(10), deg2rad(50), deg2rad(10), deg2rad(10);
    ...
    speed_distribution = std::uniform_real_distribution<double> (0.8, 1.0);
    angle_distribution = std::uniform_real_distribution<double> (deg2rad(-0), deg2rad(0));
  ...
  float step(const Eigen::Ref<EigenVec>& action) final {
    ...
    if(abs(angle_target - angle_desired) < angle_acc) angle_target = angle_desired;
    else if(angle_target > angle_desired)
    {
      angle_target -= angle_acc;
    }
    else if(angle_target < angle_desired)
    {
      angle_target += angle_acc;
  ...
  void curriculumUpdate(int update) final {
    ...
    if(update > curr1 && update < curr2)
    {
      ...
      curriculum_speed_distribution_mod(1, speed_distribution);
      curriculum_angle_distribution(1, angle_distribution);
    }
    else if(update > curr2 && update < curr3)
    {
      curriculum_speed_distribution_mod(2, speed_distribution);
      curriculum_angle_distribution(2, angle_distribution);
    }
    ...
    // {
    //   curriculum_speed_distribution_mod(4, speed_distribution);
    //   curriculum_angle_distribution(4, angle_distribution);
    // }

  }

  void change_speed_var()
  {
    unsigned seed_temp = std::chrono::system_clock::now().time_since_epoch().count();
    std::default_random_engine generator(seed_temp);
    v_desired = speed_distribution(generator);
    // change_run_stop();
  }

  void change_run_stop()
  {
    if(v_target < 0.2 && v_desired < 0.4)
    {
      mode_gait = STOP;
      v_target = 0.0;
      v_desired = 0.0;
  ...
  void change_angle_var()
  {
    unsigned seed_temp = std::chrono::system_clock::now().time_since_epoch().count();
    std::default_random_engine generator(seed_temp);
    angle_desired = angle_distribution(generator);
  }

  void mod_from_v_desired()
  {
    if(0.4 <= v_desired && v_desired <= 1.1) {
      mode_gait = WALKING_TROT;
      // v_acc = 1.2 / 30.;
    }
    else if(v_desired <= 1.7) {
      mode_gait = RUNNING_TROT;
      // v_acc = 1.8 / 30.;
    }
    else if(v_desired <= 2.0) {
      mode_gait = TRANSVERSE_GALLOP;
      // v_acc = 2.0 / 30.;
    }
    else if(v_desired <= 2.3) {
      mode_gait = BOUND;
      // v_acc = 2.3 / 30.;
    }
  }
  ...
  void visualize_arrow()
  {
    ...
    Eigen::Matrix3d aa;
    aa = Eigen::AngleAxisd(angle_target, Eigen::Vector3d(0,0,1));
    Eigen::Vector3d aaa =  aa * Eigen::Vector3d(1,0,0);
    ...
    list["ref_speed_indicator"].setScale(Eigen::Vector3d(0.1, 0.1, v_desired * 0.1));
    list["cur_speed_indicator"].setScale(Eigen::Vector3d(0.1, 0.1, v_current * 0.1));
  ...
  {   
    ...
    reward = main_w * speed_reward * dir_reward * stb_reward + torque_w * torque_reward + freq_w * freq_reward;
    return reward;
  }
  ...
  float CalcSpeedErr()
  {
    float err = 0.0;
    if(mode_gait != STOP)
      err = pow((v_current - v_target),2) / (v_desired * v_desired);
    else
  ...
  float CalcDirErr()
  {
    ...
    if(current.norm() < 1e-10)
      ...
      angle = asin(current[1]/current.norm());
    // if(abs(angle_desired) < 0.01)
    //   rot_err = (angle - angle_target)* (angle - angle_target) / 0.01;
    // else
    rot_err = (angle - angle_target)* (angle - angle_target);
  ...
  void updateObservation() {
    ...
    obScaled_[4] = angle_target;
    obScaled_[5] = angle_desired;
 ...
 private:
  ...
  double TG_phase, v_desired, v_target, v_current ,angle_prev, angle_desired, angle_current, angle_target;
  double v_acc = 1.0 / 30., angle_acc = deg2rad(15) / 30.;
  int mode_gait;
  ...
  float f_scaler = 1.0; float f_shifter = 1.1;
