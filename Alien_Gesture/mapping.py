...
from raisim_gym_m.env.bin import mapping
from raisim_gym_m.env.RaisimGymVecEnv import RaisimGymVecEnv as VecEnv
from raisim_gym_m.helper.raisim_gym_helper import ConfigurationSaver
...
# print(cfg_abs_path)
cfg_abs_path = "/home/sonic/Project/Alien_Gesture/env/AlienGo_Mapping/mapping/cfg.yaml"
...
env = VecEnv(mapping.RaisimGymEnv(rsc_path, dump(cfg['environment'], Dumper=RoundTripDumper)))

# env = Environment(RaisimGymEnv(current_dir+"/env/urdf_aliengo", dump(cfg['environment'], Dumper=RoundTripDumper)))
# Get algorithm
if mode == 'train':
    saver = ConfigurationSaver(log_dir=log_dir+'/MappingLocomotionSlowTran',
                               save_items=[rsg_root+'/env/AlienGo_Mapping/mapping/Environment.hpp', cfg_abs_path])
    model = PPO2(
    ...
    af = actionFilter(512)
    final_result = np.zeros(512)
    m_env = 10
    for epoch in range(m_env):
        went = 300
        af._ResetActionFilter()
        obs = env.reset()
        result = 300 * np.ones(512)
        flag = np.zeros(512)
        for i in range(300):
            # env.wrapper.showWindow()
    ...
    # env.kinect_init()
    obs = env.reset()
    save_file = current_dir +'/rsc/map_loco/locotrot.pt'
    contact_file = current_dir +'/rsc/map_loco/locotrotcont.pt'
    dim_input = 96
    dim_output = 16
    human_file = current_dir +'/rsc/map_loco/params_loco11.txt'
    ...
    for i in range(100000):
        ...
        # env.getContactStep(contact[i // 2, :], kin_motion[i // 2, :])
        obs, reward, done, infos = env.stepTest(action_t)
