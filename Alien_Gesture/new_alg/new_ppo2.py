...
from .new_policies import LstmPolicy, ActorCriticPolicy
...
class PPO2(ActorCriticRLModel):
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("input_info", reuse=False):
                    ...
                    tf.summary.scalar('avg_reward_per_frame', self.avg_reward)
                ...
                try:
                    if update % eval_every_n == 1:
                        ...
                        obs, returns, masks, actions, values, neglogpacs, states, ep_infos, true_reward = \
                            runner.run(test_mode=True, record_video=record_video, video_name=log_dir+"/"+str(update-1)+".mp4", update = update)
                        print("Average rewards in this test episode ", ep_infos[0]['r'])
                        model_name = log_dir + "_Iteration_{}".format(update-1)
                        self.save(model_name)
                        if ep_infos[0]['r'] > self.max_reward:
                            self.max_reward = ep_infos[0]['r']
                            self.max_save(log_dir, update-1)
                    ...
                    if self.verbose >= 1 and (update % log_interval == 0 or update == 1):
                        ...
                        if len(ep_info_buf) > 0 and len(ep_info_buf[0]) > 0:
                            rew_mean = safe_mean([ep_info['r'] for ep_info in ep_info_buf])
                            len_mean = safe_mean([ep_info['l'] for ep_info in ep_info_buf])
                            self.avg_reward = rew_mean / len_mean
                            logger.logkv('ep_reward_mean', rew_mean)
                            logger.logkv('ep_len_mean',len_mean)
    ...
    def max_save(self, save_path, numb):
        f = open(save_path + "/max_iter.txt", 'w')
        f.write("Max Reward Iterartion : {}".format(numb))
        f.close()
...
class Runner(AbstractEnvRunner):
    def __init__(self, *, env, model, n_steps, gamma, lam):
        ...
        rscdir = current_dir+ "/../rsc"
        self.mapper_model = MLP_model_tanh(self.dim_input, self.dim_output)
        file_name = rscdir + '/map_reach/All.pt'
        file_name2 = rscdir + '/map_reach/Allcont.pt'
        device = torch.device('cuda')
        self.mapper_model.load_state_dict(torch.load(file_name, map_location=device))
        self.mapper_model.cuda()
        self.mapper_model.eval()
        self.contact_model = MLP_model_tanh(self.dim_input, 4)
        self.contact_model.load_state_dict(torch.load(file_name2, map_location=device))
        self.contact_model.cuda()
        self.contact_model.eval()
        self.scaler = np.array([0.803, 2.618, 0.8905, 0.803, 2.618, 0.8905, 0.803, 2.618, 0.8905, 0.803, 2.618, 0.8905], dtype = np.float32)
        self.shifter = np.array([0., 0.716, -0.0785, 0., 0.716,  -0.0785, 0., 0.716, -0.0785, 0., 0.716, -0.0785], dtype = np.float32)
        self.real = np.array([0., 0.855, -1.728, 0., 0.855, -1.728, 0., 0.855, -1.728, 0., 0.855, -1.728], dtype = np.float32)

    def run(self, test_mode=False, record_video=False, video_name="", update = 0):
        """
        ...
        self.env.curriculum_callback(update)
        ...
        for _ in range(self.n_steps):
            ...
            # Here, predict the new reference pose using a learned mapper
            # It is the same with stepping.
            human_pose = self.obs[:,:96]
            X = torch.FloatTensor(human_pose).to(device)
            y_result = self.mapper_model.forward(X).cpu().detach().numpy()
            contact = self.contact_model.forward(X).cpu().detach().numpy()
            kin_motion = self.np_result_parser(y_result).astype(np.float32)

            # Here, put scaled result ==> use perAgentTarget
            self.env.getMappedTarget(kin_motion, contact)
    ...
    def np_result_parser(self, result):
        epsilon = 1e-6
        ori0_mtx = np.block([[np.identity(4)] ,[np.zeros((12,4))]])
        pose0_mtx = np.block([[np.zeros((4,12))], [np.identity(12)]])
        ori_0 = np.matmul(result, ori0_mtx)
        norm_quat0 = np.sqrt(np.matmul(np.power(ori_0 ,2), np.ones((4,1))))
        quat_result0 = ori_0 / (norm_quat0 + epsilon) 

        joint_result0 = np.matmul(result, pose0_mtx)
        scaled_residual0 = np.add(np.multiply(self.scaler, joint_result0), self.shifter)
        scaled_angle0 = scaled_residual0 + self.real

        result = np.concatenate((quat_result0, scaled_angle0), axis = 1)
        return result
