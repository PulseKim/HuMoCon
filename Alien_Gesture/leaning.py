...
from raisim_gym_lean.env.bin import lean
from raisim_gym_lean.env.RaisimGymVecEnv import RaisimGymVecEnv as VecEnv
from raisim_gym_lean.helper.raisim_gym_helper import ConfigurationSaver
...
# print(cfg_abs_path)
cfg_abs_path = "/home/sonic/Project/Alien_Gesture/env/AlienGo_Lean/lean/cfg.yaml"
...
env = VecEnv(lean.RaisimGymEnv(rsc_path, dump(cfg['environment'], Dumper=RoundTripDumper)))

# Get algorithm
if mode == 'train':
    saver = ConfigurationSaver(log_dir=log_dir+'/Lean2021',
                               save_items=[rsg_root+'/env/AlienGo_Lean/lean/Environment.hpp', cfg_abs_path])
    model = PPO2(
    ...
    model.set_env(env)
    model.set_update(10000)

    # tensorboard
    # Make sure that your chrome browser is already on.
    TensorboardLauncher_local(saver.data_dir + '/PPO2_1')
    # PPO run
    model.learn(total_timesteps=1400000000, eval_every_n=cfg['environment']['eval_every_n'], log_dir=saver.data_dir, record_video=False)
    ...
    env.kinect_init()
    obs = env.reset()
    save_file = current_dir +'/rsc/map_lean/tilt1312.pt'
    contact_file = current_dir +'/rsc/map_lean/tiltcont.pt'
    dim_input = 96
    dim_output = 16
    # human_file = current_dir +'/rsc/map_lean/params_leanlong.txt'
