...
from raisim_gym_push.env.bin import env
from raisim_gym_push.env.RaisimGymVecEnv import RaisimGymVecEnv as VecEnv
from raisim_gym_push.helper.raisim_gym_helper import ConfigurationSaver

from raisim_gym_push.algo.ppo2 import PPO2
from raisim_gym_push.archi.policies import MlpPolicy
...
# print(cfg_abs_path)
cfg_abs_path = "/home/sonic/Project/Alien_Gesture/env/A1_Reaching/env/cfg.yaml"
...
# Get algorithm
if mode == 'train':
    saver = ConfigurationSaver(log_dir=log_dir+'/MappingReaching',
                               save_items=[rsg_root+'/env/A1_Reaching/env/Environment.hpp', cfg_abs_path])
    model = PPO2(
    ...
    model.set_env(env)
    model.set_update(19300)

    # tensorboard
    # Make sure that your chrome browser is already on.
    TensorboardLauncher_local(saver.data_dir + '/PPO2_1')
    # PPO run
    model.learn(total_timesteps=1000000000, eval_every_n=cfg['environment']['eval_every_n'], log_dir=saver.data_dir, record_video=False)
