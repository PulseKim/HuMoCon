...
public:
  ...
  mRobot(robot_), fps(fps_){
    ...
    JointVelocityLimit.push_back(0.10);
    JointVelocityLimit.push_back(0.17);
    JointVelocityLimit.push_back(0.087);
    JointVelocityLimit.push_back(0.052);
    JointVelocityLimit.push_back(0.12);
    JointVelocityLimit.push_back(0.026);
    threshold = 0.5;
    twoside = -1;
    // 0 : Over / 1: Contact
    contactCFlag = Eigen::Vector4i(1,1,1,1);
    contactCFlagPrev = Eigen::Vector4i(1,1,1,1);
    startFlag = true;
  }
  ...
  void setPrevPose(Eigen::VectorXd pose_)
  {
    prev_kin_pose = pose_;
  }
  ...
  void compensateWalk()
  {
    ...
    mRobot->setGeneralizedCoordinate(raw_pose);
    for(int i =0; i < 4; ++i)
    {
      raisim::Vec<3> foot;
      mRobot->getFramePosition(FR_FOOT + 3*i, foot);
    ...
    Eigen::Vector4i pinned = contactCFlag.cwiseProduct(contactCFlagPrev);
    float delX = 0;
    int cnt = 0;
    if(pinned.norm() == 0) delX = 0;
    else{
      for(int i = 0; i <4; ++i)
      {
        int curr = pinned[i];
    ...
    prev_pose[0] -= max(delX, 0.0f);
  }

  Eigen::VectorXd velocity_adjustment(Eigen::VectorXd src)
  {
    float max_dev; 
    for(int i = 7; i < src.size(); ++i)
    {
      max_dev = 2. * JointVelocityLimit[(i-1)%3];
      float dev = src[i] - prev_kin_pose[i];
      if(dev > max_dev) src[i] = prev_kin_pose[i] + max_dev;
      else if(-max_dev > dev) src[i] = prev_kin_pose[i] - max_dev;
    }
    return src;
  }

  Eigen::VectorXd velocity_adjustmentManipulate(Eigen::VectorXd src)
  {
    float max_dev; 
    for(int i = 7; i < src.size(); ++i)
    {
      max_dev = 0.5 * JointVelocityLimit[(i-1)%6];
      float dev = src[i] - prev_kin_pose[i];
      if(dev > max_dev) src[i] = prev_kin_pose[i] + max_dev;
      else if(-max_dev > dev) src[i] = prev_kin_pose[i] - max_dev;
    }
  ...
  void getKinematicFixedManipulate()
  {
    Eigen::VectorXd mid_result = raw_pose;
    // dynContactAdjustment();
    // mid_result = attractCOM(mid_result);
    mid_result = contactConsistency(mid_result);
    mid_result = velocity_adjustmentManipulate(mid_result);
    // Finalize the solution
    reshaped_pose = mid_result;
    prev_kin_pose = reshaped_pose;
  }
  ...
  float getJointLimit(int idx)
  {
    return JointVelocityLimit[idx];
  }
...
private:
  ...
  std::vector<float> JointVelocityLimit;
  bool startFlag;
  int gcDim_;
  float fps;
  ...
  Eigen::Vector4i contactCFlagPrev;
